...
#define LOG_DEBUG_SEND_MESSAGE_IF( condition, ... ) \
    if( condition ) \
    { \
        LOG_DEBUG_SEND_MESSAGE( __VA_ARGS__ ); \
    }

#define LOG_INFO_SEND_MESSAGE_IF( condition, ... ) \
    if( condition ) \
    { \
        LOG_INFO_SEND_MESSAGE( __VA_ARGS__ ); \
    }

#define LOG_WARN_SEND_MESSAGE_IF( condition, ... ) \
    if( condition ) \
    { \
        LOG_WARN_SEND_MESSAGE( __VA_ARGS__ ); \
    }

#define LOG_ERROR_SEND_MESSAGE_IF( condition, ... ) \
    if( condition ) \
    { \
        LOG_ERROR_SEND_MESSAGE( __VA_ARGS__ ); \
    }

#define LOG_FATAL_SEND_MESSAGE_IF( condition, ... ) \
    if( condition ) \
    { \
        LOG_FATAL_SEND_MESSAGE( __VA_ARGS__ ); \
    }
